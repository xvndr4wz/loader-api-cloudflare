...
export async function onRequest(context) {
    ...
    try {
        ...
        if (currentStep === 0) {
            ...
            if (rateData) {
                ...
                if (elapsed < SETTINGS.RATE_LIMIT_MS) {
                    rateData.count++;
                    // Gunakan await yang ketat untuk memastikan data tersimpan
        ...
        // ========== CEK REPLAY ATTACK (SEBELUM LOGIKA LAIN) ==========
        ...
        if (idx === SETTINGS.TOTAL_LAYERS - 2) {
            ...
            // Mark current session as used SEBELUM membuat yang baru
            session.used = true;
            await kv.put(`session:${id}`, JSON.stringify(session), { expirationTtl: 600 });
